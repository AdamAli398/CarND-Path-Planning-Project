...
int check_lane(double d, int lane) {
  for (int i = 0; i < 3; i++) {
...
int main() {
  ...
  h.onMessage([&map_waypoints_x,&map_waypoints_y,&map_waypoints_s,
               &map_waypoints_dx,&map_waypoints_dy,&ref_vel,&lane,&lane_count]
    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          bool remain_in_lane = true;
          bool too_close = false; // True if too close to a car in front
          bool left_lane_blocked = false, right_lane_blocked = false;
          int next_lane = lane;
          if (lane_count > 0) {
            lane_count--;
          } else {
            remain_in_lane = false;
          }
          ...
          for (int i = 0; i < sensor_fusion.size(); i++) {
            ...
            if (d < (2+4*lane+2) && d > (2+4*lane-2)){
              ...
              left_lane_blocked = lane == 0 ? true : left_lane_blocked;
              right_lane_blocked = lane == 2 ? true : right_lane_blocked;

              double min_dist = 20;
              double future_s = ((double)prev_size * 0.02 * check_speed) + check_car_s;

              int potential_lane = check_lane(d, lane);
              if (future_s < car_s - min_dist || future_s > car_s + min_dist) {
                if (potential_lane == lane - 1 && !left_lane_blocked) {
                  next_lane = lane - 1;
                } else if (potential_lane == lane + 1 && !right_lane_blocked) {
                  next_lane = lane + 1;
                }
              } else {
                if (potential_lane == lane - 1) {
          ...
          if (too_close && ref_vel > 42.0 && !remain_in_lane && next_lane != lane && (!left_lane_blocked || !right_lane_blocked)) {
            lane = next_lane;
            lane_count = 120;
          }
